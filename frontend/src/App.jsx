...
const emptyProduct = { name: "", description: "", price: "", category: "", brand: "", stock: "" };
...
function Navbar({ user, isAdmin, search, setSearch, cartCount, onCartClick, onLogout, onHome, onOrders }) {
  return (
...
function Sidebar({ categories, active, onSelect }) {
  return (
    <div style={{ width: 190, flexShrink: 0, background: "#121212", border: "1px solid #262626", borderRadius: 12, padding: 16, height: "fit-content" }}>
      <h4 style={{ color: "#f0f0f0", margin: "0 0 12px" }}>Categories</h4>
      {["All", ...categories].map((c) => (
        <div key={c} onClick={() => onSelect(c === "All" ? "" : c)} style={{
          padding: "8px 10px", borderRadius: 8, cursor: "pointer", fontSize: 14, marginBottom: 4,
          color: (active === c || (!active && c === "All")) ? "#fff" : "#999",
          background: (active === c || (!active && c === "All")) ? "#7c5cff" : "transparent",
        }}>
          {c}
        </div>
      ))}
...
function ProductCard({ p, isAdmin, onView, onEdit, onDelete }) {
  return (
    <div onClick={() => onView(p)} style={{ background: "#161616", border: "1px solid #262626", borderRadius: 12, padding: 14, cursor: "pointer" }}>
      ...
      <p style={{ margin: "0 0 8px", color: "#888", fontSize: 13 }}>{p.category} • Stock: {p.stock}</p>
    ...
    <Card wide>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginTop: 20 }}>
        ...
        <div style={{ textAlign: "left" }}>
          ...
          <p style={{ color: "#888", fontSize: 13, margin: "0 0 4px" }}>Category: {p.category}</p>
...
function CartPage({ cart, onBack, onRemove, onBuyNow }) {
  ...
  return (
    <Card wide>
      ...
      {cart.length === 0 ? (
        <p style={{ color: "#888" }}>Cart is empty. Go add some products!</p>
      ) : (
        <>
          {cart.map((p, i) => (
...
function PaymentPage({ items, onBack, onPay, loading }) {
  ...
  const handlePay = (e) => {
    e.preventDefault();
    if (!address.fullName || !address.phone || !address.address) return;
    onPay(items, address);
  };
    ...
    <Card wide>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
        <form onSubmit={handlePay} style={{ textAlign: "left" }}>
          ...
          <Button type="submit" variant="buy" loading={loading} style={{ width: "100%", marginTop: 12 }}>
            Pay ₹{total}
          </Button>
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", color: "#f0f0f0", fontWeight: 700, fontSize: 16 }}>
  ...
  return (
    <Card wide>
      ...
      {orders.length === 0 ? (
        ...
        orders.map((o) => (
          <div key={o._id} style={{ background: "#161616", border: "1px solid #262626", borderRadius: 10, padding: 14, marginBottom: 12, textAlign: "left" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ color: "#f0f0f0", fontWeight: 600 }}>Order #{o._id.slice(-6)}</span>
              <span style={{ color: "#7de3a4", fontSize: 13 }}>{o.orderStatus}</span>
            </div>
            {o.items.map((it, i) => (
              <p key={i} style={{ color: "#ccc", fontSize: 13, margin: "2px 0" }}>{it.name} x {it.quantity} — ₹{it.price * it.quantity}</p>
            ))}
...
export default function App() {
  ...
  const fetchProducts = async () => {
    try {
      const data = await apiJSON(PRODUCT_API, "/", null, "GET");
  ...
  const openEditForm = (p) => {
    setProductForm({ name: p.name, description: p.description, price: p.price, category: p.category, brand: p.brand || "", stock: p.stock });
    setImageFiles([]);
    setEditingId(p._id);
    setShowForm(true);
  };
  ...
  const handleSaveProduct = async (e) => {
    e.preventDefault();
    const { name, description, price, category, stock } = productForm;
    if (!name || !description || !price || !category) return showToast("Please fill all fields", "error");
    setLoading(true);
    try {
  ...
  const handleDeleteProduct = async (id) => {
    ...
    try {
      await apiJSON(PRODUCT_API, `/${id}`, null, "DELETE", true);
  ...
  const fetchOrders = async () => {
    try {
      const data = await apiJSON(ORDER_API, "/my", null, "GET", true);
  ...
  const filteredProducts = products.filter(
    (p) => (!category || p.category === category) && p.name.toLowerCase().includes(search.toLowerCase())
  );
        ...
        <Card>
          ...
          {page === "register" && (
            <form onSubmit={handleRegister}>
              ...
              <p style={{ textAlign: "center", fontSize: 13, color: "#888", marginTop: 18 }}>
                Already have an account? <span onClick={() => !loading && setPage("login")} style={{ color: "#7c5cff", cursor: "pointer", fontWeight: 600 }}>Log in</span>
              </p>
            </form>
          )}
          ...
          {page === "verify-otp" && (
            <form onSubmit={handleVerifyOtp}>
              ...
              <p style={{ textAlign: "center", fontSize: 13, color: "#888", marginTop: 18 }}>
                Didn't get the code? <span onClick={() => !loading && handleResendOtp()} style={{ color: "#7c5cff", cursor: "pointer", fontWeight: 600 }}>Resend OTP</span>
              </p>
            </form>
          )}
          ...
          {page === "login" && (
            <form onSubmit={handleLogin}>
              ...
              <p style={{ textAlign: "center", fontSize: 13, color: "#888", marginTop: 18 }}>
                New here? <span onClick={() => !loading && setPage("register")} style={{ color: "#7c5cff", cursor: "pointer", fontWeight: 600 }}>Create an account</span>
              </p>
            </form>
          )}
        </Card>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <Navbar
        ...
        onLogout={handleLogout} onHome={() => setPage("dashboard")} onOrders={openOrders}
      />

      <div style={{ display: "flex", gap: 20, padding: 24, alignItems: "flex-start", justifyContent: page === "dashboard" ? "flex-start" : "center" }}>
        {page === "dashboard" && <Sidebar categories={categories} active={category} onSelect={setCategory} />}
        ...
        <div style={{ flex: page === "dashboard" ? 1 : "unset" }}>
              ...
              {showForm && (
                <form onSubmit={handleSaveProduct} style={{ background: "#161616", border: "1px solid #262626", borderRadius: 12, padding: 20, marginBottom: 24 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    ...
                    <Field label="Category" name="category" value={productForm.category} onChange={onChange(setProductForm)} />
              ...
              {filteredProducts.length === 0 ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 16 }}>
                  {filteredProducts.map((p) => (
                    <ProductCard key={p._id} p={p} isAdmin={isAdmin} onView={openProductDetail} onEdit={openEditForm} onDelete={handleDeleteProduct} />
                  ))}
                </div>
              )}
            </>
          ...
          {page === "product-detail" && selectedProduct && (
            <ProductDetail p={selectedProduct} onBack={backToDashboard} onAddToCart={handleAddToCart} onBuyNow={goToPayment} />
          )}

          {page === "cart" && (
            <CartPage cart={cart} onBack={backToDashboard} onRemove={handleRemoveFromCart} onBuyNow={goToPayment} />
          )}

          {page === "payment" && (
            <PaymentPage items={checkoutItems} onBack={() => setPage("dashboard")} onPay={handlePay} loading={loading} />
          )}

          {page === "order-success" && (
            <OrderSuccessPage order={lastOrder} onContinue={backToDashboard} />
          )}

          {page === "orders" && (
            <OrdersPage orders={orders} onBack={backToDashboard} />
          )}
        </div>
      </div>
    </div>
