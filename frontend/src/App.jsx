...
export default function App() {
  ...
  // restore login session from localStorage on refresh/reload
  useEffect(() => {
  ...
  const handleLogout = () => {
    setUser(null); setToken(null); setCart([]); setWishlist([]); setRegisterForm(emptyRegister); setOtpForm(emptyOtp); setLoginForm(emptyLogin); setPage("dashboard");
  ...
  // ---- Products (public, no auth needed) ----
  const fetchProducts = async () => { setProductsLoading(true); try { const data = await apiJSON(PRODUCT_API, "/", null, "GET"); setProducts(data.products || []); } catch (err) { showToast(err.message, "error"); } finally { setProductsLoading(false); } };
  useEffect(() => { if (page === "dashboard") fetchProducts(); }, [page]);
        ...
        <div style={{ flex: "1 1 300px", minWidth: 0 }}>
          {page === "dashboard" && (
            <>
              ...
              {productsLoading ? (
                <p style={{ color: "#888", textAlign: "center" }}>⏳ Loading products, please wait...</p>
              ) : filteredProducts.length === 0 ? <p style={{ color: "#888", textAlign: "center" }}>No products found.</p> : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
                  {filteredProducts.map((p) => <ProductCard key={p._id} p={p} isAdmin={isAdmin} wishlisted={isWishlisted(p._id)} onView={openProductDetail} onEdit={openEditForm} onDelete={handleDeleteProduct} onToggleWishlist={handleToggleWishlist} />)}
                </div>
