...
function Navbar({ user, isAdmin, search, setSearch, cartCount, onCartClick, onLogout, onHome }) {
  return (
    <div style={{
      position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", gap: 16,
      background: "#161616", borderBottom: "1px solid #262626", padding: "12px 24px",
    }}>
      ...
      <input
        ...
        style={{ ...inputStyle, marginBottom: 0, flex: 1, maxWidth: 480 }}
      />
      <div style={{ flex: 1 }} />
      <span style={{ color: "#aaa", fontSize: 13 }}>{user?.name} • {isAdmin ? "Admin" : "Customer"}</span>
      <button onClick={onCartClick} style={{
        position: "relative", background: "transparent", border: "none", color: "#f0f0f0",
        fontSize: 22, cursor: "pointer",
      }}>
        🛒
        {cartCount > 0 && (
          <span style={{
            position: "absolute", top: -6, right: -10, background: "#ff5c9e", color: "#fff",
            borderRadius: "50%", fontSize: 11, width: 18, height: 18, display: "flex",
...
function ProductDetail({ p, onBack, onAddToCart, onBuy }) {
  return (
    <Card wide>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginTop: 20 }}>
        ...
        <div style={{ textAlign: "left" }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <Button variant="buy" style={{ flex: 1 }} onClick={() => onBuy(p)}>Buy Now</Button>
          </div>
        </div>
      </div>
    </Card>
...
function CartPage({ cart, onBack, onRemove, onBuy }) {
      ...
      {cart.length === 0 ? (
        ...
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
            ...
            <Button variant="buy" onClick={onBuy}>Checkout</Button>
          </div>
        </>
      )}
    </Card>
...
export default function App() {
  const [page, setPage] = useState("register"); // register | verify-otp | login | dashboard | product-detail | cart
  ...
  const handleBuy = () => showToast("Proceeding to checkout (coming soon)");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <Navbar
        ...
        onLogout={handleLogout} onHome={() => setPage("dashboard")}
      />
      ...
      <div style={{ display: "flex", gap: 20, padding: 24, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {page === "product-detail" && selectedProduct && (
            <ProductDetail p={selectedProduct} onBack={backToDashboard} onAddToCart={handleAddToCart} onBuy={handleBuy} />
          )}

          {page === "cart" && (
            <CartPage cart={cart} onBack={backToDashboard} onRemove={handleRemoveFromCart} onBuy={handleBuy} />
          )}
        </div>
      </div>
    </div>
